...
/* ============================================================
   ПУЛ ФОНОВЫХ ИЗОБРАЖЕНИЙ (Unsplash, board-game тематика)
   Используется когда у игры нет персональной обложки.
   Распределяется по (id % IMG.length).
============================================================ */
const IMG = [
...
/* ============================================================
   БАЗА ДАННЫХ ИГР
   Источник: «Список настолок 25.12.2024.csv» — только базовые игры.
   Категории: family | strategy | hardcore | coop | deluxe
   Цена:  BYN за сутки аренды.
   Рейтинг: личная оценка из CSV (0 = не выставлена).
============================================================ */
const games = [
  /* ── ДЕЛЮКС-ИЗДАНИЯ ─────────────────────────────────────── */
...
/* Скидка за длительность: от 3 дней — −15% */
const DISCOUNT_THRESHOLD = 3;
const DISCOUNT_RATE      = 0.15;

/* ============================================================
   DOM — ссылки на элементы из index.html
============================================================ */
...
/* ============================================================
   ПОИСК — инжектируем инпут в каталог
============================================================ */
...
/* ============================================================
   КАЛЬКУЛЯТОР — заполнение select и живой пересчёт
============================================================ */
...
/* ============================================================
   ФОРМА БРОНИРОВАНИЯ — поля Имя / Телефон
============================================================ */
...
rentalForm.addEventListener('submit', e => {
  ...
  if (!gameOk || !daysOk || !nameOk || !phoneOk) {
    ...
    openModal({
      type: 'error',
      body: `<ul class="modal__error-list">
  ...
  openModal({
    type: 'success',
    body: `
      <p>Игра <strong>${escapeHtml(game.name)}</strong> забронирована на
      <strong>${days} ${pluralDays(days)}</strong> для <strong>${escapeHtml(name)}</strong>.</p>
      ${discountHtml}
      ...
      <p class="modal__note">
        📞 Свяжемся по номеру <strong>${escapeHtml(phone)}</strong><br />
        📍 г. Минск, ул. Примерная, д. 1, офис 101
      </p>
    `,
  });
...
/* ============================================================
   HEADER — класс .scrolled при прокрутке
============================================================ */
